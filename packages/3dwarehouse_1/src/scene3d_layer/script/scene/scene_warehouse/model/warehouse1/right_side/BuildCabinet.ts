...
export let build = (state: state, cabinetNumber: number) => {
    ...
    // let drawerMaterial = NewThreeInstance.createMeshPhysicalMaterial({
    //     color: getCabinetDrawerColor(),
    //     metalness: 0.6,
    //     roughness: 0.9
    // })
    ...
    // let cabinetDrawer = BuildCabinetDrawer.build(state, drawerMaterial)
    let cabinetDrawer = getDrawer(state)
    // cabinetDrawer.traverse((obj: Mesh) => {
    ...
    //     }
    // })
    // // cabinetDrawer.scale.multiplyScalar(9.5)
