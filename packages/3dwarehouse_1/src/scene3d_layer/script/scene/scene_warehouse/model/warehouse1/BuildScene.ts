...
let _addCabinets = (state: state, scene,) => {
    ...
    // let cabinet = BuildRightSideCabinet.build(state, 1)
...
let _addInstances = (state: state, scene, dynamicGroup) => {
    let data = Instance.convertToInstanceMesh(getAbstractState(state), scene, findAllDrawers(dynamicGroup))
...
export let build = (state: state, renderer) => {
    let staticGroup = NewThreeInstance.createGroup()
    staticGroup.name = getStaticGroupName()
    let dynamicGroup = NewThreeInstance.createGroup()
    dynamicGroup.name = getDynamicGroupName()

    let scene = NewThreeInstance.createScene()
