...
let Nav: React.FC = () => {
    let mode = useSelector<AppState>((state) => state.warehouse.mode)
    ...
    let _renderButtons = (sceneNumber) => {
        return WarehouseScene3D.getAllSceneNumbers()
        .filter(value => value != sceneNumber)
        .map(value => {
            return (
        ...
        switch (mode) {
            ...
            default:
                switch (currentScene) {
                    ...
                    //     return (
                    //         <>
                    //             <Button className="tab" type="primary" onClick={_ => _switchScene(currentScene, scene.Warehouse, 1)}>
                    //                 仓库1
                    //             </Button>
                    //             <Button className="tab" type="primary" onClick={_ => _switchScene(currentScene, scene.Warehouse, 2)}>
                    //                 仓库2
                    //             </Button>
                    //         </>
                    //     )
                    case scene.Warehouse:
                    default:
                        return (<>{_renderButtons(sceneNumber)} </>)
                }
        }
    }
