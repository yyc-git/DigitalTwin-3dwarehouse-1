...
import { findInstance, findSourceMesh } from './Instance';
...
export class InstancedMeshBVH {
  ...
  private setup(state:state): Float32Array {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const bbox = _box.copy(bboxGeometry).applyMatrix4(findSourceMesh(state, this._target.name, i).matrixWorld)
